...
RSpec.describe Feedback::BehindTheGems4 do
  subject { Feedback::BehindTheGems4.new }

  it "#title returns correct title" do
    expect(subject.title).to eq "Behind the Gems 4 :: 6"
  end
  ...
  it "#submit return true if all required answers are provided" do
    expect(
      Feedback::BehindTheGems4.new(ActionController::Parameters.new({ rating: 1 })).submit
    ).to eq true
  end

  it "#submit creates a FormResponse if all required answers are provided" do
    form = Feedback::BehindTheGems4.new(ActionController::Parameters.new({ rating: 1 }))
