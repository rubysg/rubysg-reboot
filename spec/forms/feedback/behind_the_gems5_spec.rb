...
RSpec.describe Feedback::BehindTheGems5 do
  subject { Feedback::BehindTheGems5.new }

  it "#title returns correct title" do
    expect(subject.title).to eq "Behind the Gems 5 :: 6"
  end
  ...
  it "#submit return true if all required answers are provided" do
    expect(
      Feedback::BehindTheGems5.new(ActionController::Parameters.new({ rating: 1 })).submit
    ).to eq true
  end

  it "#submit creates a FormResponse if all required answers are provided" do
    form = Feedback::BehindTheGems5.new(ActionController::Parameters.new({ rating: 1 }))
